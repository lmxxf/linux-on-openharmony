...
如果内网机没有外网，但通过 USB 连着 OH 设备，而 OH 设备通过 WiFi 和你的互联网机在同一局域网，可以借 OH 做跳板，从互联网机远程桌面到内网机：

```
互联网机 --WiFi--> OH 设备 --USB(hdc)--> 内网机
```

**前置条件：**
- 内网机开启远程桌面（设置 → 系统 → 远程桌面 → 打开）
- 内网机上 hdc 已连接 OH 设备
- 互联网机能 SSH 到 OH 设备的 Alpine

**步骤：**

1. **内网机上**（PowerShell），用 hdc 反向端口转发，把 OH 的 3389 映射到内网机的 3389：

```powershell
hdc rport tcp:3389 tcp:3389
```

2. **互联网机上**（PowerShell），SSH 到 OH 并建立本地端口转发：

```powershell
ssh -L 13389:127.0.0.1:3389 root@<OH设备IP>
```

3. **互联网机上**，打开远程桌面（mstsc），连接 `127.0.0.1:13389`，即可看到内网机桌面。

> 用 13389 而不是 3389，是因为互联网机本地的 3389 可能被自己的远程桌面服务占用。

**取消反向端口转发：**

```powershell
# 内网机上执行
hdc rport rm tcp:3389 tcp:3389
```
